...
## Bilateral-Async-Unified Run Notes

- For strict M11 governance/deployment validation, the fallback RPC `https://services.polkadothub-rpc.com/testnet/` worked reliably for historical `eth_call` checks on `GovernancePolicyStore`, `AccessManager`, `LendingEngine.previewBorrow`, and receipt/log inspection.
- `VAL-BILATERAL-003`, `VAL-BILATERAL-004`, and `VAL-BILATERAL-007` are provable read-only from the canonical M11 manifest plus fallback-RPC checks; the key governance execute tx is `0x58da3cdfc88ed7f3c409cb2f5bdd00867043bd4213bee3a3809699d658a2746a` and the policy update tx is `0x479f3d8cc8620e784ed02628c483e35da944af79955db3c598eb67e688862ea2`.
- The current canonical M11 async-proof artifacts do **not** prove a connected live async trace: liquidation tx `0xa1ad2ca7c7ada4cee6daa13e11866ae56ec8bc7b57b7a302c7837c39f2083b03` emitted `HookFailed`, no `LiquidationNotified` event was observed for the proof window, the committed inbox receipt tx `0xb5eeb3530a9968a7c062e346771da5f6246128fb546daf3d8383b496ac167499` used correlationId `0xf393acd4...` instead of the liquidation correlationId `0xca4aaead...`, and `scripts/event-correlator.ts --historical` produced 0 correlations for that window.
- Strict M11 rename/doc audits should not rely only on README tables or `SYSTEM_ARCHITECTURE_MERMAID.md`: the active source tree still contains `contracts/LendingCore.sol`, and stale pre-M11 naming/claims remain in `SYSTEM_ARCHITECTURE_AUDIT.md`, `README.md`, and `docs/dualvm/dualvm_submission_final.md`.
- Strict `VAL-BILATERAL-001` evaluation must check more than top-level canonical filenames: active interfaces/identifiers still retain legacy names such as `IRiskAdapter`, `IMigratableLendingCore`, and `lendingCore` fields/functions, and non-Foundry deployment entrypoints still exist (`script/DeployPVM.sh`, `scripts/deploy-deterministic-risk-model-pvm.mjs`, `scripts/deployNotifierDirect.mjs`, and `package.json` `deploy:pvm:riskmodel:standalone`).
- Strict `VAL-BILATERAL-008` evaluation currently fails because `SYSTEM_ARCHITECTURE_AUDIT.md` still says XCM is standalone / not connected to lending, and `docs/dualvm/dualvm_submission_final.md` still uses pre-M11 names (`LendingCore`, `RiskAdapter`, `PvmQuoteProbe`) despite README and `SYSTEM_ARCHITECTURE_MERMAID.md` being mostly aligned.
