...
/// @title LendingCoreV2
/// @notice V2 of LendingCore with two additions over the base contract:
///
/// 1. `depositCollateralFor(address beneficiary, uint256 amount)`: same logic as
///    `depositCollateral` but credits `beneficiary`'s position instead of msg.sender.
///    Restricted to approved routers via AccessManager (uses `restricted` modifier).
///
/// 2. Optional post-liquidation hook: after `_liquidateOneFrom` completes, if
///    `liquidationNotifier` is non-zero, calls `ILiquidationNotifier.notifyLiquidation()`
///    wrapped in try/catch so notifier failures never revert the liquidation.
///
/// All existing LendingCore functions are unchanged.
contract LendingCoreV2 is AccessManaged, Pausable, ReentrancyGuard, IMigratableLendingCore {
    ...
    constructor(
        ...
        liquidationNotifier = liquidationNotifier_;
    }
    ...
    function publishCurrentQuoteTicket(address borrower) external restricted returns (bytes32 ticketId) {
    ...
    /// @dev Transfers `amount` of collateral from msg.sender (the router) and credits
    /// `beneficiary`'s position — msg.sender's own position is unaffected.
    /// Restricted: only callable by approved routers wired via AccessManager.
    /// @param beneficiary The address whose collateral position is credited.
    /// @param amount Amount of collateral asset to transfer and credit.
    ...
    function _liquidateOneFrom(address liquidator, address borrower, uint256 requestedRepayAmount) internal {
        ...
        if (liquidationNotifier != address(0)) {
            try ILiquidationNotifier(liquidationNotifier)
                .notifyLiquidation(borrower, actualRepay, collateralSeized, bytes32(0)) {}
                catch {}
        }
    }
