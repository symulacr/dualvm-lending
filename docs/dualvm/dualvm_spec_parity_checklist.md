...
| Official public testnet target | Original spec named Passet Hub `420420422` | Implementation retargeted to current official Polkadot Hub TestNet `420420417` | DRIFTED | `dualvm/hardhat.config.ts`, `dualvm/lib/config/marketConfig.ts`, `CLAUDE.md` | Low if documented honestly |
...
| AccessManager architecture | Modern access stack rather than single owner | AccessManager deployed and function-role mapping configured | DONE structurally | `dualvm/contracts/DualVMAccessManager.sol`, `dualvm/lib/deployment/deploySystem.ts` | Medium if readers assume live role separation from architecture alone |
