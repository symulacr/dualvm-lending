...
import type { ExtensionUIContext, ExtensionUIDialogOptions, ExtensionWidgetOptions } from "../extensibility/extensions";
...
export class InteractiveMode implements InteractiveModeContext {
	private static readonly MAX_HOOK_WIDGET_LINES = 10;
	...
	readonly #hookWidgetsAbove = new Map<string, Component & { dispose?(): void }>();
	readonly #hookWidgetsBelow = new Map<string, Component & { dispose?(): void }>();
	...
	clearHookWidgets(): void {
		...
		this.#renderHookWidgets();
	}

	#renderHookWidgets(): void {
		this.#renderHookWidgetContainer(this.widgetContainerAbove, this.#hookWidgetsAbove, true, true);
		this.#renderHookWidgetContainer(this.widgetContainerBelow, this.#hookWidgetsBelow, false, false);
		this.ui.requestRender();
	}
	...
	setHookWidget(
		...
		const removeExisting = (widgets: Map<string, Component & { dispose?(): void }>) => {
			const existing = widgets.get(key);
			existing?.dispose?.();
			widgets.delete(key);
		};

		removeExisting(this.#hookWidgetsAbove);
		removeExisting(this.#hookWidgetsBelow);
		...
		if (Array.isArray(content)) {
			...
			for (const line of content.slice(0, InteractiveMode.MAX_HOOK_WIDGET_LINES)) {
				container.addChild(new Text(line, 1, 0));
			}
			if (content.length > InteractiveMode.MAX_HOOK_WIDGET_LINES) {
		...
		const targetWidgets = placement === "belowEditor" ? this.#hookWidgetsBelow : this.#hookWidgetsAbove;
		targetWidgets.set(key, component);
		this.#renderHookWidgets();
	}
