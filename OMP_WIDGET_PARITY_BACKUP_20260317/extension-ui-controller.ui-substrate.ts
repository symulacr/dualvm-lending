...
export class ExtensionUiController {
	...
	setHookWidget(key: string, content: unknown, options?: ExtensionWidgetOptions): void {
		this.ctx.setHookWidget(
			key,
			content as string[] | ((tui: TUI, theme: Theme) => Component & { dispose?(): void }) | undefined,
			options,
		);
	}
	...
	async showHookCustom<T>(
		...
		const keybindings = this.ctx.keybindings;
		...
		return new Promise((resolve, reject) => {
			...
			Promise.resolve(factory(this.ctx.ui, theme, keybindings, close))
				.then(c => {
					if (closed) {
						c.dispose?.();
						return;
					...
					component = c;
					if (isOverlay) {
						const resolvedOptions = options?.overlayOptions
						...
						overlayHandle = this.ctx.ui.showOverlay(component, resolvedOptions);
